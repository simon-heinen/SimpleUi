...
public abstract class M_Double implements ModifierInterface, UiDecoratable {
	...
	public View getView(Context context) {
		...
		e.setKeyListener(new NumberKeyListener() {
		...
		double numberToDisplay = load();
		if ((int) numberToDisplay == numberToDisplay) {
			// dont show the comma if not needed
			e.setText("" + (int) numberToDisplay);
		} else {
			e.setText("" + numberToDisplay);
		}
	...
	public boolean save() {
		...
		try {
			String textToParse = e.getText().toString();
			textToParse = textToParse.replace(",", ".");
			return save(Double.parseDouble(textToParse));
