package customViews.header2actionbar;
 ...
 * <p>
 * <b>Important</b>: Use {@link android.R.id#background} to specify background
 * view and {@link android.R.id#title} to specify view on top of the header (for
 * example: a shadow for {@code ActionBar}).
 * <p>
 */
@SuppressLint("NewApi")
public abstract class HeaderFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (content instanceof ListView) {
			...
			listView.setOnScrollListener(new AbsListView.OnScrollListener() {
				...
				public void onScrollStateChanged(AbsListView absListView,
					...
					if (mOnScrollListener != null) {
						mOnScrollListener.onScrollStateChanged(absListView,
								scrollState);
					}
				}
				...
				public void onScroll(AbsListView absListView,
						...
						int totalItemCount) {
					if (mOnScrollListener != null) {
						mOnScrollListener.onScroll(absListView,
					...
					if (isListViewEmpty) {
						...
						assert child != null;
						scrollHeaderTo(child == mFakeHeader ? child.getTop()
								: -mHeaderHeight);
					}
				}
			});
			...
			final NotifyingScrollView scrollView = new NotifyingScrollView(
					activity);
	 ...
	 */
	public void setListViewOnScrollChangedListener(
